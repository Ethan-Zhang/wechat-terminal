...
IRCHandler.prototype._common_handler = function(command, args) {
  ...
  switch(command) {
    ...
    case 'JOIN':
      ...
      if(!(channel in wxClient.members)) {
    ...
    case 'PART':
      ...
      this._socket.write(sprintf(":%s!%s@%s %s %s :%s\r\n", this._nick, this._user, '127.0.0.1', 'PART', '#'+server.curChannel, this._nick));
      break;
    case 'NOTICE':
    case 'PRIVMSG':
      if (args.length<1) {
      ...
      name = server.curChannel;
      msg = args[1];
      //this._socket.write(sprintf(":%s!%s@%s %s %s :%s\r\n", this._nick, this._user, '127.0.0.1', 'PRIVMSG', '#'+server.curChannel, msg));
      wxClient.send_msg(name, msg);
      break;
    default:
...
ChatRoomMsgHandleWxPlugin.prototype._handle_msg_hook(msgType, userName, remarkName, msgId, content) {
  this.add_msg(remarkName, msgId, content);
  if(this._irc_handler.curChannel == remarkName)
    this.notify(remarkName);
}

ChatRoomMsgHandleWxPlugin.prototype.add_msg(remarkName, msgId, content) {
  if(!(remarkName in this._queues))
    this._queues[remarkName] = [];
  this._queues[remarkName].push({'msgId':msgId, 'content':content});
}

ChatRoomMsgHandleWxPlugin.prototype.notify() {
  while(this._queues[remarkName].length) {
    var msg_info = this._queues[remarkName].pop();
    if(msg_info[msgId] == 1)
      this._irc_handler.send(msg_info[content]);
    else {
      this._irc_handler.send("暂不支持显示的消息");
    }
  }
}
